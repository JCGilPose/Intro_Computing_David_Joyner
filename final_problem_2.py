...
#Add your code here!
def generate_name(filename, person):
    hero_names = open(filename, 'r')
    person_names = person.split()
    initial_1 = person_names[0][0]
    initial_2 = person_names[1][0]
    hero_id = ''
    count = 64
    for line in hero_names:
        name = line.rsplit()
        count += 1
        if 64 < count <= 90 and chr(count) == initial_1:
            hero_id += ((' ').join(name) + ' ')
        elif chr(count - 26) == initial_2:
            hero_id += (' ').join(name)
    hero_names.close()
    return hero_id
