...
class Force:
    ...
    def get_angle(self, use_degrees=True):
        if use_degrees == True:
            return degrees(atan2(self.get_vertical(), self.get_horizontal()))
        return atan2(self.get_vertical(), self.get_horizontal())
...
def find_net_force(forces):
    ...
    for force in forces:
        ...
        vertic += force.get_vertical()
    net_magnitude = (horiz ** 2 + vertic ** 2) ** 0.5
    net_magnitude = round(net_magnitude, 1)
    net_angle = degrees(atan2(vertic, horiz))
    net_angle = round(net_angle, 1)
    net_force = Force(net_magnitude, net_angle)
    return net_force
